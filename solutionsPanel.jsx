...
class SolutionsPanel extends React.Component {
    ...
    if (this.props.userType) {
      if (selectedCustomer) {
        if (this.props.solutionsData === "") {
          ...
          if (
            selectedCustomer &&
            selectedCustomer !== prevState.selectedCustomer
          ) {
            this.setState({ selectedCustomer: selectedCustomer });
            this.setState({ solutions: this.getDashboardsState() }, () =>
  ...
  render() {
    ...
    if (document.getElementById("solutionsSearch")) {
      let solutionsSearchClearButton = document.getElementById(
        "solutionsSearchClearButton"
      );
    ...
    return (
      <div id="solutionsPanel">
        <div className="title-container">
          ...
          <button
            type="button"
            className="btn btn-link close-panel-button"
        ...
        <div className="tabs-container">
          <div className="right-controls">
            <div className="view-by-container">
              ...
              <div
                tabIndex="0"
                className="view-by-dropdown-button"
                onClick={() => this.toggleTypeOptionsDropdown()}
              >
                <strong className="view-by-state">
                  {this.state.viewByType}
                </strong>
            ...
            <div className="sort-by-container">
              ...
              <div
                tabIndex="0"
                className="sort-by-dropdown-button"
                onClick={() => this.toggleSortOptionsDropdown()}
              >
                <strong className="sort-by-state">
                  {this.state.sortByProperty}
                </strong>
                <i className="fas fa-chevron-down" />
              </div>
            </div>

            <div className="view-by-dropdown">{typeRows}</div>
            <div className="sort-by-dropdown">{sortRows}</div>
          </div>
        </div>
        <div className="search-container">
          <input
            id="solutionsSearch"
            className="era-solutions-search form-control search-input"
            type="text"
            placeholder="Search"
            aria-label="Search"
          ...
          <button
            type="button"
            id="solutionsSearchClearButton"
            className="era-solutions-search-clear-button btn bg-transparent"
        ...
        <div className="era-solutions-table">
          {this.state.isError ? (
            ...
            <div>Please wait reports are Loading...</div>
          ) : (
            solutionsRows
          )}
        </div>
  ...
  handleOutsidePanelClick() {
    ...
    const leftNavCollapseButton = document.getElementById(
    ...
    if (solutionPanel && leftNavCollapseButton) {
      ...
      window.addEventListener("click", function (e) {
        ...
        let clickIsOutsideOfLeftNavCollapseButton =
          !leftNavCollapseButton.contains(e.target);
        let clickIsOutsideOfAnchor = !document
          .getElementById("solutionsPanelAnchor")
          .contains(e.target);
        let panelIsOpen = false;

        if (solutionPanel.getAttribute("style")) {
          panelIsOpen = solutionPanel
            .getAttribute("style")
            .includes("display: grid;");
        }

        if (
          clickIsOutsideOfPanel &&
          clickIsOutsideOfLeftNavCollapseButton &&
          clickIsOutsideOfAnchor &&
          panelIsOpen
        ) {
          context.closePanel();
        }
    ...
    if (searchTerm !== "") {
      let searchFiltered = solutions.filter((solution) => {
        ...
        if (solution.description) {
          ...
          return (
            lcName.includes(searchFilter) ||
            lcDescription.includes(searchFilter)
          );
        } else {
  ...
  getDashboardsState() {
    ...
    if (this.props.userType) {
      ...
      if (selectedCustomer) {
        return axios
          .get("/looker/get/solutions?customerName=" + selectedCustomer)
          .then((response) => {
            let types = [{ name: "All", color: "#FFFFFF" }];
  ...
  generateSolutionRows() {
    ...
    for (let solution of this.state.filteredSolutions) {
      solutionsElements.push(
        <a
          href={`/#/solutions/${solution.id}-${solution.name}`}
          onClick={() =>
            this.solutionClick(`/solutions/${solution.id}-${solution.name}`)
          }
          className="era-solution-row"
          tabIndex="0"
        >
          <div className="era-solution-name">{solution.name}</div>
          <div className="era-solution-type">
            <i
              className="fas fa-chart-bar era-solution-type-color"
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <p>
                      This Modal should open to guide_url from table solutions
                    </p>
                  </div>
          ...
          <div class="container">
            <button
              ...
              data-target="#myModal"
            >
              <VideoPlayerFilled width="24px" height="24px" />{" "}
            </button>
            ...
            <div class="modal fade" id="myModal" role="dialog">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <p>
                      This Modal should open to VIDO_URL from table solutions.
                    </p>
                  </div>
                  <div class="modal-footer">
                    <button
  ...
  generateTypeRows() {
    ...
    for (let type of this.state.types) {
      typeRows.push(
        <div
          tabIndex="0"
          className="era-dropdown-option"
          onClick={() => this.filterByType(type.name)}
        >
          <div
            className="view-by-type-color"
